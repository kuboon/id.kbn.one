...
const decodeBase64Url = (value: string): Uint8Array => {
  ...
  const normalized = sanitized + "=".repeat(padding);
...
const encodeBase64Url = (bytes: Uint8Array): string =>
  fromArrayBuffer(toArrayBuffer(bytes), true);

const normalizeSubscriptionKey = (
  key: string,
  field: "auth" | "p256dh",
): string => {
  if (!key || typeof key !== "string") {
    throw new Error(`Subscription ${field} key is invalid`);
  }
...
const decodeStoredSubscriptionKey = (value: string): ArrayBuffer =>
  toArrayBuffer(decodeBase64Url(value));
...
export interface StoredPushSubscription {
  ...
  endpoint: string;
  expirationTime: number | null;
  keys: PushSubscriptionPayload["keys"];
  createdAt: string;
  updatedAt: string;
...
export class PushService {
  ...
  private validateSubscriptionPayload(
    ...
    const normalizedKeys = {
      auth: normalizeSubscriptionKey(keys.auth, "auth"),
      p256dh: normalizeSubscriptionKey(keys.p256dh, "p256dh"),
    };
    return {
      endpoint,
  ...
  async sendNotification(
    ...
    const subscriber = this.applicationServer.subscribe({
      ...
      keys: {
        auth: decodeStoredSubscriptionKey(subscription.keys.auth),
        p256dh: decodeStoredSubscriptionKey(subscription.keys.p256dh),
      },
    });
