/**
 * Adapted from https://github.com/negrel/http-ece (MIT License).
 */
import {
  bytes,
  decodeBase64,
...
export class Header {
  ...
  public toBytes(): ArrayBuffer {
    const bytesArr = new Uint8Array(this.byteLength);
    const dv = new DataView(bytesArr.buffer);

    bytesArr.set(this.salt);
    dv.setUint32(SALT_LENGTH, this.rs);
    dv.setUint8(SALT_LENGTH + RS_LENGTH, this.idlen);
    bytesArr.set(this.keyid, SALT_LENGTH + RS_LENGTH + IDLEN_LENGTH);
