...
Deno.test("client and server together validate DPoP proof", async () => {
	...
	const fakeFetch = async (input: RequestInfo, init?: RequestInit) => {
		...
		const result = await verifyDpopProofFromRequest(req, { checkReplay: async () => true });
		if (!result.valid) {
	...
	const res = await apiCall(url, { method: "PUT", headers: { "X-Client": "1" } });
	...
	if (recorded.length !== 1) throw new Error("expected server to be called once");
