...
Deno.test("apiCall attaches DPoP header and preserves other headers", async () => {
  ...
  if (!call.init || !call.init.headers) {
    throw new Error("expected headers in fetch init");
  }
  ...
  if (parts.length !== 3) {
    throw new Error("DPoP proof is not a JWT-like string");
  }
  ...
  if (headerJson.typ !== "dpop+jwt") {
    throw new Error("unexpected typ in header");
  }
  ...
  if (payloadJson.htm !== "POST") {
    throw new Error("method not normalized in payload");
  }
  if (payloadJson.htu !== "https://example.com/some/path?x=1") {
    throw new Error("htu not normalized in payload");
  }
  if (typeof payloadJson.jti !== "string") {
    throw new Error("jti missing or wrong type");
  }
  if (typeof payloadJson.iat !== "number") {
    throw new Error("iat missing or wrong type");
  }
